...
import 'package:gunter/service/google_sheet_service.dart';
...
class _SettingGeneralPageState extends State<SettingGeneralPage> {
  ...
  void localSettingTempPath() async {
    final file = File('$_settingPath/path.cfg');
  ...
  void getActivate() async {
    final File activateFile = File('$_settingPath/activate.aes256');
      ...
      try {
        ...
        if (tokenObj != null) {
          if (localUpdatedAt != tokenObj.updatedAt ||
            ...
            if (_settingPath != null) {
              final file = File('${_settingPath!}/activate.aes256');
  ...
  void onSelect() async {
    ...
    file.filterSpecification = {'file': 'RebirthClient.exe'};
    final result = file.getFile();
    if (result != null && _settingPath != null) {
      final file = File('${_settingPath!}/path.cfg');
      final path = result.path.split('RebirthClient.exe')[0];
