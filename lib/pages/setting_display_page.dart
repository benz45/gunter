...
import 'package:gunter/business/convert_xml_business.dart';
import 'package:gunter/business/directory_business.dart';
import 'package:gunter/business/page_business.dart';
import 'package:gunter/business/screenRetriever_business.dart';
import 'package:gunter/components/box_detail.dart';
import 'package:gunter/components/setting_display_page/custom_dropdown.dart';
import 'package:gunter/components/setting_display_page/premium_box.dart';
import 'package:gunter/components/setting_display_page/screen_mode_radio.dart';
import 'package:gunter/components/setting_display_page/screen_size_dropdown.dart';
import 'package:gunter/components/snack_bar_alert.dart';
...
class _SettingDisplayPageState extends State<SettingDisplayPage> {
  ...
  void onUpdateSetting(ProfileConfig profileConfig) async {
    ...
    final path = Directory('${directory.path}/rebirth-setting');
    if (await path.exists() && path.path != null) {
      final String settingPath = path.path;

      final tempFile = File('$settingPath/path.cfg');
      final isTempFileExits = await tempFile.exists();
      if (isTempFileExits) {
        final pathConfigContent = await tempFile.readAsString();
        String? settingConfigPath = pathConfigContent;
        if (settingConfigPath != null) {
          try {
            settingConfigPath = settingConfigPath.replaceAll(r'\', r'/');
            final fileOption = File('${settingConfigPath}option.xml');
            final isFileOptionExits = await tempFile.exists();
            if (isFileOptionExits) {
              final contentXML = await fileOption.readAsString();
              if (contentXML != '') {
                final xmlString = contentXML;
                final document = xml.XmlDocument.parse(xmlString);
                final rootElement = document.rootElement;
                final xml.XmlElement? graphic =
                    rootElement.getElement('graphic');
                ...
                await fileOption.writeAsString(
            ...
            final logFile = File('$settingPath/error-log.log');
            await logFile.writeAsString(e.toString());
          }
        }
      }
    }
  ...
  void onSubmit() async {
    ...
    if (setting != null) {
      ...
      if (_settingProvider.getProfileSelected == 1) {
        setting.profile1 = profileConfig;
      } else if (_settingProvider.getProfileSelected == 2) {
        setting.profile2 = profileConfig;
      } else if (_settingProvider.getProfileSelected == 3) {
